...
export interface EsbuildHtmlOptions {
  /**
   * Base directory used for resolving the HTML template if it's specified as a relative path.
   *
   * @default 'process.cwd()'
   */
  basedir?: string;
   ...
   * specified in the HTML template. CSS files that are referenced directly in HTML and those that
   * are referenced indirectly from JS entry points will be included.
   */
